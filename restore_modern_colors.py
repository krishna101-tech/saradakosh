...
# The original modernization block, but adding the collapse arrows and pointer cursor
modern_style = """<style>
...
for file in os.listdir(reports_dir):
    if file.endswith(".html"):
        ...
        # Replace the first style block (the main styles) with the restored modern styles
        new_content = re.sub(r'<style>.*?</style>', modern_style, content, count=1, flags=re.DOTALL)
        
        if new_content != content:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(new_content)
            print(f"Restored modern colors in {file}")
